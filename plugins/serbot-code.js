...
let handler = async (m, { conn: _conn, args, usedPrefix, command, isOwner }) => {
    ...
    async function serbot() {
        ...
        if (methodCode && !conn.authState.creds.registered) {
            ...
            // Verificación de PHONENUMBER_MCC para evitar errores
            if (!PHONENUMBER_MCC || typeof PHONENUMBER_MCC !== 'object') {
                console.error("PHONENUMBER_MCC no es un objeto válido. Verifica su definición.");
                process.exit(0);
            }

            if (!Object.keys(PHONENUMBER_MCC).some(v => cleanedNumber.startsWith(v))) {
                console.error("El número no coincide con ningún código en PHONENUMBER_MCC.");
                process.exit(0);
            }
