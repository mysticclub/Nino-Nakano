...
let senderCoo = users[senderId].corazones || 0
let randomUserId = Object.keys(users)[Math.floor(Math.random() * Object.keys(users).length)]
while (randomUserId === senderId) {
randomUserId = Object.keys(users)[Math.floor(Math.random() * Object.keys(users).length)]}
let randomUserCoo = users[randomUserId].corazones || 0
let minAmount = 15
let maxAmount = 50
...
text: `🚩¡Te Postituiste Y Ganaste *${amountTaken} corazones 🤍* Dejaste Casi Seco A @${randomUserId.split("@")[0]}\n\nSe suman *+${amountTaken} corazones 🤍* a ${senderName}.`,
contextInfo: { 
...
conn.reply(m.chat, `🚩 Le Rompiste La Verga A Tu Cliente Te Cobro Y Se Te Quitan *-${amountSubtracted} corazones 🤍* a ${senderName}.`, m, rcanal)
break
case 2:
...
conn.sendMessage(m.chat, {
text: `🚩 Vuelves A Las Calles Y Te Vas A Un Motel Te Paga ${smallAmountTaken} corazones 🤍* de @${randomUserId.split("@")[0]}\n\nSe suman *+${smallAmountTaken} corazones 🤍* a ${senderName}.`,
contextInfo: { 
mentionedJid: [randomUserId],
}}, { quoted: m })
break
