...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  setTimeout(() => {
    ...
    if (win) {
      ...
      conn.reply(m.chat, `🚩 ¡Ganaste! Obtuviste ${cookies} *🤍 corazones*. Total: ${users.cookies} *🤍 corazones*.`, m, rcanal)
    } else {
      users.cookies -= cookies
      conn.reply(m.chat, `🚩 Perdiste. Se restaron ${cookies} *🤍 corazones*. Total: ${users.cookies} *🤍 corazones*.`, m, rcanal)
    }
