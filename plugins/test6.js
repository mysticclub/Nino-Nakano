...
const handler = async (m, { conn }) => {
  ...
  const welcomer = new canvacard.WelcomeLeave()
    .setAvatar(img)
    .setBackground('IMAGE', background)
    .setTitulo("NUEVO INGRESO AL GRUPO", '#FFFFFF') // Título con color blanco
    .setSubtitulo("Por favor leer las reglas del grupo", '#FFFFFF') // Subtítulo con color blanco
    .setOpacityOverlay(0.5) // Ajusta la transparencia del overlay
    .setColorCircle('#FFFFFF')
    .setColorOverlay('rgba(255, 255, 255, 0.5)'); // Overlay blanco semitransparente
  ...
  // Genera la tarjeta
  try {
    const buffer = await welcomer.build(); // Usa el estilo de fuente predeterminado
    const filePath = './output/WelcomeCard.png';
